<template>
  <div class="container" v-if="!showDetail">
    <div class="header">
      <div class="title" style=" text-align: left;">未审核职位</div>
    </div>
...
export default {
  ...
  methods: {
    async getByState() {
      ...
      // 未审核：state=0
      const positions = await Position.getByState(0, page, count)
    ...
    async handlePass(val) {
      const res = await Position.updateState(val.id, 1)
    ...
    async handleFail(val) {
      const res = await Position.updateState(val.id, 2)
      if (res.code < window.MAX_SUCCESS_CODE) {
        this.$message.success(`${res.message}`)
        // 刷新页面
        this.getByState()
