  ...
  <el-dropdown :trigger="trigger" :placement="placement" :hide-on-click="hideOnClick">
    ...
    <el-dropdown-menu slot="dropdown">
      ...
      <div class="content" :style="{ 'min-height': height + 'px', 'max-height': height + 'px' }">
        ...
        <el-dropdown-item v-for="(msg, index) in messages" :key="index" @click.native="readMessages()">
          <slot :row="msg">
            <p class="read-messages" v-if="msg.is_read === 'true'">{{ msg[props.content] }}</p>
            <p class="unread-messages" v-else>{{ msg[props.content] }}</p>
